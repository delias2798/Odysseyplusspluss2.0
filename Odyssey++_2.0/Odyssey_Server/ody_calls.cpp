...
QString ody_calls::answer(QString xml)
{
    ...
    if(tag=="NeedTrack")
        return needTrack(root);

    if(tag=="DelTrack")
        return delTreack(root);

    if(tag=="SetRate")
        return setRate(root);

    if(tag=="AddVideo")
        return addVideo(root);

    if(tag=="DelVideo")
        return delVideo(root);

    if(tag=="NeedVideo")
        return needVideo(root);

    if(tag=="NeedAllVideos")
        return needAllVideo(root);

    if(tag=="SetRateVideo")
        return setRateVideo(root);

    if(tag=="UpdateVideo")
        return updateVideo(root);
...
QString ody_calls::needTrack(QDomElement root)
{
    string user_, title_;
    user_=xmlS.getData2(root,"username");
    title_=xmlS.getData2(root,"title");
    calls *sql = new calls();
...
QString ody_calls::setRate(QDomElement root)
{
    string user, title, rate;
    user=xmlS.getData2(root,"username");
    title=xmlS.getData2(root,"title");
    rate=xmlS.getData2(root,"rate");
    calls *sql = new calls();
    if(sql->setRate(user,title,rate))
        return "TRUE";
    return "FALSE";
}

QString ody_calls::addVideo(QDomElement root)
{
    string user, title, path, descr;
    user=xmlS.getData2(root,"username");
    title=xmlS.getData2(root,"title");
    descr=xmlS.getData2(root,"letter");
    path=xmlS.getData2(root,"path");
    calls *sql = new calls();
    if(sql->addVideo(title,"/",path,user))
        return "TRUE";
    return "FALSE";
}

QString ody_calls::delVideo(QDomElement root)
{
    string user, title;
    user=xmlS.getData2(root,"username");
    title=xmlS.getData2(root,"title");
    calls *sql = new calls();
    if(sql->deleteVideo(title,user))
        return "TRUE";
    return "FALSE";
}

QString ody_calls::needVideo(QDomElement root)
{
    string user, title;
    user=xmlS.getData2(root,"username");
    title=xmlS.getData2(root,"title");
    calls *sql = new calls();
    return sql->needVideo(user,title);
}

QString ody_calls::needAllVideo(QDomElement root)
{
    string user;
    user=xmlS.getData2(root,"username");
    calls *sql = new calls();
    return sql->needAllVideo(user);
}

QString ody_calls::setRateVideo(QDomElement root)
{
    string user, title, rate;
    user=xmlS.getData2(root,"username");
    title=xmlS.getData2(root,"title");
    rate=xmlS.getData2(root,"rate");
    calls *sql = new calls();
    if(sql->setRateV(user,title,rate))
        return "TRUE";
    return "FALSE";
}

QString ody_calls::updateVideo(QDomElement root)
{
    string user, title, des, dir;
    user=xmlS.getData2(root,"username");
    title=xmlS.getData2(root,"title");
    des=xmlS.getData2(root,"letter");
    dir=xmlS.getData2(root,"path");

    calls *sql = new calls();
    if(sql->updateVideo(title,des,dir,user))
        return "TRUE";
    return "FALSE";
}
...
QString ody_calls::addTrack(QDomElement root)
{
    ...
    direction=xmlS.getData2(root,"path");
    calls *sql = new calls();
...
QString ody_calls::delTreack(QDomElement root)
{
    string user_, title_;
    user_=xmlS.getData2(root,"username");
    title_=xmlS.getData2(root,"title");
    calls *sql = new calls();
    if(sql->deleteSong(title_,user_))
        return "TRUE";
    return "FALSE";
}
